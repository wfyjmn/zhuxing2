...
def screen_stocks(market_regime):
    ...
    try:
        ...
        df_daily_basic = api_call_with_retry(
            pro.daily_basic,
            trade_date=trade_date,
            fields='ts_code,pe_ttm,total_mv,circ_mv,turnover_rate'  # 直接获取换手率
        )
    ...
    try:
        ...
        if len(df_hist) > 0:
            ...
            print("    ⚠️  未获取到历史数据，使用默认成交量倍数")

        # 使用daily_basic提供的换手率（单位：%）
        # 如果没有换手率数据，则使用计算值
...
def main():
    """主函数"""
    print("=" * 80)
    print("AI辅助短线选股程序（选股A）- 优化版")
    print("=" * 80)
    print(f"\n当前时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("优化内容：")
    print("  - 添加API重试机制和请求间隔")
    print("  - 优化换手率计算，直接从daily_basic获取")
    ...
    # 步骤2：筛选股票
    df_result = screen_stocks(market_regime)

    # 步骤3：输出结果
    output_results(df_result, market_regime, strategy)
